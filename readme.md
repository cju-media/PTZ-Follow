...
A Node.js server that tracks a subject using a PTZ camera via the VISCA over IP protocol. It features a web GUI for drawing a tracking bounding box over an SRT video stream and supports OSC commands for integration with other software.
...
- **Low Latency Video:** Transcodes SRT to MPEG1 using `fluent-ffmpeg` and streams to the browser via WebSockets (JSMpeg).
